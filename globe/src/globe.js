...
export class Terra5Globe {
    ...
    _makeIcon(emoji, size = 48) {
        ...
        const cached = globeCache.iconCache.get(key);
        ...
        globeCache.iconCache.set(key, dataUrl, true);
        return dataUrl;
    }
    ...
    init() {
        ...
        this.viewer.scene.globe.maximumMemoryUsage = 256; // Balanced memory budget - prevents thrashing while keeping RAM under control
    ...
    setEcoMode(enabled) {
        ...
        if (enabled) {
            this.viewer.targetFrameRate = 30;
            const dpr = window.devicePixelRatio || 1;
            this.viewer.resolutionScale = Math.min(0.75, 0.75 / (dpr > 1.2 ? 1.25 : 1.0));
            if (this.viewer.scene.globe) {
                this.viewer.scene.globe.maximumScreenSpaceError = 3.5;
                this.viewer.scene.globe.tileCacheSize = 40;
                this.viewer.scene.globe.maximumMemoryUsage = 64; // Low 64MB RAM budget in eco
                this.viewer.scene.globe.preloadAncestors = false;
                this.viewer.scene.globe.preloadSiblings = false;
            }
        } else {
            ...
            if (this.viewer.scene.globe) {
                ...
                this.viewer.scene.globe.tileCacheSize = 100;
                this.viewer.scene.globe.maximumMemoryUsage = 256;
                this.viewer.scene.globe.preloadAncestors = false;
                this.viewer.scene.globe.preloadSiblings = false;
            }
        }
    ...
    pruneMemory() {
        ...
        try {
            // NEVER call clearTileCache() while Cesium is active:
            // clearTileCache() unloads visible tiles and forces an immediate reload storm!
            // Instead, let Cesium's internal LRU respect maximumMemoryUsage.
            if (globeCache && globeCache.iconCache) {
                globeCache.iconCache.clearNonPinned();
            }
            this._visibleContactsCache = null;
            console.log('[Cesium] Globe memory pruned safely without tile thrashing');
        } catch (e) {
    ...
    updateFlights(flights) {
        ...
        if (radarDs) radarDs.entities.suspendEvents();
        try {
            ...
            const maxFlt = this._ecoMode ? 250 : 800;
            const flightList = (flights || []).slice(0, maxFlt);
            ...
            if (radarDs) radarDs.entities.resumeEvents();
        }
    ...
    updateVessels(vessels) {
        ...
        try {
            ...
            const maxVsl = this._ecoMode ? 150 : 450;
            const vesselList = (vessels || []).slice(0, maxVsl);
    ...
    updateSatellites(satellites) {
        ...
        try {
            ...
            const maxSats = this._ecoMode ? 100 : 350;
            const satList = (satellites || []).slice(0, maxSats);
            satList.forEach(s => {
    ...
    removeCCTV(camId) {
        if (!this.viewer || !this.dataSources.cctv) return;
        const normalizedId = String(camId).startsWith('cctv-') ? camId : `cctv-${camId}`;
        const ds = this.dataSources.cctv;
        const entity = ds.entities.getById(normalizedId);
        if (entity) {
    ...
    _createVesselSVG(color = '#00d4aa') {
        ...
        globeCache.iconCache.set(key, url, true);
        return url;
    }
    ...
    async loadArgusLandmarks(category = null) {
        ...
        try {
            const geojson = await globeCache.fetchWithCache(url, 300000);
            if (!geojson) return;
    ...
    getVisibleContactsInWindow() {
        ...
        // 0. Cache Check: Instant return if camera is stationary and cache is fresh (<1.8s)
        const camPos = camera.positionWC;
        const nowPerf = performance.now();
        if (this._visibleContactsCache && (nowPerf - this._visibleContactsCacheTime < 1800)) {
            if (this._visibleContactsCachePos && Cesium.Cartesian3.distanceSquared(camPos, this._visibleContactsCachePos) < 2.0) {
                return this._visibleContactsCache;
            }
        }
        ...
        const MAX_WINDOW_CONTACTS = 120;

        for (const key of layerKeys) {
            if (contacts.length >= MAX_WINDOW_CONTACTS) break;
        ...
        this._visibleContactsCache = payload;
        this._visibleContactsCacheTime = nowPerf;
        this._visibleContactsCachePos = Cesium.Cartesian3.clone(camPos);
