...
export class CyberIncidentEngine {
  ...
  /**
   * Evaluates an event against incident creation rules.
   * @param {Object} event
   * @param {number} scenarioStage
   * @returns {Object|null}
   */
  evaluateEventForIncident(event, scenarioStage = 1) {
    ...
    // Check for open incident for this node
    for (const incident of this.activeIncidents.values()) {
      if (incident.primaryNodeId === event.sourceId && incident.status !== 'RESOLVED') {
        ...
        // Align lifecycle state with stage progression
